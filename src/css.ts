...
export function namedElements(viewTransitionRoot: HTMLElement = document.documentElement) {
  ...
  [...document.styleSheets, ...document.adoptedStyleSheets].forEach(sheet => namedElementsOfSheet(sheet));
...
function namedElementsOfRule(rule: CSSRule, keyframeName?: string) {
  if (rule instanceof CSSImportRule || rule instanceof CSSFontFaceRule || rule instanceof CSSPageRule || rule instanceof CSSNamespaceRule || rule instanceof CSSKeyframesRule || rule instanceof CSSCounterStyleRule || rule instanceof CSSFontFeatureValuesRule || rule instanceof CSSFontPaletteValuesRule || rule instanceof CSSLayerStatementRule || rule instanceof CSSPropertyRule || window["CSSViewTransitionRule"] && rule instanceof CSSViewTransitionRule) return;
...
function styledElements(parent: CSSRule | null) {
  ...
  for (; ;) {
    ...
    selectors.forEach(sel => {
      ...
      const parsed = parse(sel, { context: 'selector' });
      ...
      [...document.querySelectorAll<HTMLElement>(sel)].filter(el => root.contains(el)).forEach(element => elements.push({ element, pseudoElement }));
    });
    break;
  }
}
...
function collect(parent: CSSStyleRule | CSSScopeRule, selectors: string[]): string[] {
  ...
  if (parent instanceof CSSStyleRule) {
    splitSelectorList(parent.selectorText).forEach(sel => nested.forEach(nes => selectors.push(nes.replaceAll(/&/g, sel))));
  } else {
    const scopes = parent.start ? splitSelectorList(parent.start) : [deriveCSSSelector(parent.parentStyleSheet?.ownerNode?.parentElement ?? document.documentElement)];
...
const splitSelectorList = (s: string): string[] => {
  return parse(s, { context: 'selectorList' }).children.map(node => generate(node));
};
