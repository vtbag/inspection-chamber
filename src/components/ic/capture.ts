...
function capture(
	...
	named.forEach(({ element, pseudoElement }) => {
		...
		if (style.viewTransitionName !== 'none' && style.display === 'none') return;
		...
		const node = {
			...
			viewTransitionScope: style.viewTransitionScope === 'auto' ? ("" + ++scopeCount) : style.viewTransitionScope,
			style,
			children: [],
		};
		sparseDOM.push(node);
		...
		function cssUnescape(str: string): string {
			return str?.replace(/\\([0-9a-fA-F]{1,6})\s?/g, (_match, hex) => String.fromCharCode(parseInt(hex, 16)))?.replace(/\\(.)/g, '$1');
		}
