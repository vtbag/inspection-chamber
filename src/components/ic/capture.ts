...
function capture(
	...
	let rootNode = sparseDomNode(transitionRoot);
	rootNode.hiddenBecause?.delete('viewTransitionScope');
	rootNode.hiddenBy = rootNode.hiddenBecause?.size ? rootNode : undefined;
	...
	named.forEach(({ element, pseudoElement }) => {
		...
		const potentiallyRenamed = renameAutoGeneratedNames(style.viewTransitionName);
		const node = sparseDomNode(element, pseudoElement, style);
		if (potentiallyRenamed === 'none' && !node.hiddenBy) return;
		node.pseudoElement = pseudoElement;
		node.viewTransitionName = potentiallyRenamed;
		node.viewTransitionGroup = style.viewTransitionGroup;
		node.viewTransitionClass = style.viewTransitionClass;
		node.viewTransitionScope = style.viewTransitionScope;
		...
		function renameAutoGeneratedNames(viewTransitionName: string): string {
			const name = cssUnescape(viewTransitionName);
			if (name === 'auto' && !CSS.supports('view-transition-name: auto')) return 'none';
			if (name === 'match-element' && !CSS.supports('view-transition-name: match-element'))
				return 'none';
			if (name === 'auto' && element.id) return `-ua-id-${element.id}`;
			if (name === 'match-element' || name === 'auto') {
				const idKey = `vtbag-auto-name-id${pseudoElement ? pseudoElement : ''}`;
				let id = (element as any)[idKey] as string | undefined;
				if (!id) {
					(element as any)[idKey] = id = `-vtbag-${name}-${autoCount}-${++idCount}`;
				}
	...
	if (nestGroups(rootNode, groupRoot, groupRoot, groups, oldOrNew, capture)) {
		message(
			'error',
...
['ic-after-capture-old', 'ic-after-capture-new', 'ic-ready-error'].forEach((eventName) =>
	self.addEventListener(eventName, (event) => {
		...
		root.ownerDocument.head.querySelector(
			`#vtbag-ic-temp-style-${gid(moduleGroupMaps.get(root)?.get('@')!)}`
		);
		//?.remove();
	})
);
